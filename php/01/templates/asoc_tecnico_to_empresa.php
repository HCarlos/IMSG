        ...
        <div class="span4">
            <div class="panel panel-success">
              ...
              <div class="panel-body" style="height: 30em !important; ">
                    <select class="lstTécnicos " name="lstTécnicos" id="lstTécnicos" multiple="multiple" style="width:100% !important; height: 95% !important;" >
                    </select>
...
function getEmpresas(){
    ...
    $("#selEmpresas").empty();
    $.post(obj.getValue(0)+"data/", { o:31, t:100, p:51, c:nc,from:0,cantidad:0, s:"" },
        function(json){
...
function getTécnicos(){
    var nc = "u="+localStorage.nc;
    $.post(obj.getValue(0)+"data/", { o:31, t:103, p:51, c:nc,from:0,cantidad:0, s:"" },
        function(json){
            var nc;
           $.each(json, function(i, item) {
                //alert(item.label);
                nc = item.label.split("@");
                $("#lstTécnicos").append('<option value="'+item.data+'"> '+item.label+'</option>');
            });
            $("#lbl02").html(commaSeparateNumber(json.length)+" Técnicos")
...
function getTeclsEmp(){
    ...
    $.post(obj.getValue(0)+"data/", {o:31, t:104, p:51,c:nc,from:0,cantidad:0, s:y },
        function(json){
           $.each(json, function(i, item) {
                $("#lstTecnicos").append('<option value="'+item.data+'"> '+item.label+'</option>');
            });
            $("#lbl01").html(commaSeparateNumber(json.length)+" Técnicos");
        }, "json"
    );
}
...
$("#chkAllGru").on("click",function(event){
    ...
    if (checked) {
        $("#lstTécnicos option").each(function(){
...
$("#AddItem").on("click",function(event){
    ...
    var x = $('.lstTécnicos option:selected').val();
    var y = $('select[name="selEmpresas"] option:selected').val();
    ...
    if (isDefined(x)){
        ...
        x='';
        $(".lstTécnicos option:selected").each(function () {
                x += $(this).val() + "|";
          });
    ...
    $.post(obj.getValue(0)+"data/", { o:31, c:d, t:10, p:53, s:nc, from:0, cantidad:0 },
        function(json){
            //alert( json[0].msg);
...
$("#DeleteItem").on("click",function(event){
    ...
    var x = $('.lstTecnicos option:selected').val();
    ...
    if (isDefined(parseInt(x))){
        ...
        x='';
        $(".lstTecnicos option:selected").each(function () {
                x += $(this).val() + "|";
          });
    ...
    $.post(obj.getValue(0)+"data/", { o:31, c:x, t:20, p:53, s:nc, from:0, cantidad:0 },
        function(json){
            //alert(json[0].msg)
            if (json.length<=0 && json[0].msg=="Error") { return false;}
                //getEmpresas();
