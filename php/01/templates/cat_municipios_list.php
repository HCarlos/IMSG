...
jQuery(function($) {
	...
	function fillTable(){
		...
		$.post(obj.getValue(0) + "data/", {o:2, t:3, c:nc, p:55, from:0, cantidad:0,s:''},
			function(json){
					...
					$.each(json, function(i, item) {
						...
						tB +='						</a>';
						tB +='	';
						tB +='						<a class="red delMunicipio" href="#"  id="delMunicipio-'+item.idmunicipio+'" >';
					...
					$(".modMunicipioPro").on("click",function(event){
						event.preventDefault();
						var arr = event.currentTarget.id.split('-');
						obj.setIsTimeLine(false);
						getPropMunicipio(arr[1]);
					});

					$(".delMunicipio").on("click",function(event){
						event.preventDefault();
						...
						if (resp){
							...
							//alert(arr[1]);
							obj.setIsTimeLine(false);
				            $.post(obj.getValue(0) + "data/", {o:2, t:2, c:arr[1], p:52, from:0, cantidad:0, s:''},
				            function(json) {
				            		if (json[0].msg=="OK"){
										onClickFillTable();
				        			}else{
	...
	function getPropMunicipio(IdMunicipio){
		var nc = localStorage.nc; //.split("@");
		$.post(obj.getValue(0) + "cat-municipios-prop/", {
				user: nc,
	...
	stream.on("servidor", jsNewMunicipio);
	function jsNewMunicipio(datosServer) {
