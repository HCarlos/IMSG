	...
	<div id="user-profile-1" class="user-profile row-fluid">
			<div id="sample-table-2_wrapper" class="dataTables_wrapper" role="grid">
				...
				<table aria-describedby="sample-table-2_info" id="sample-table-2" class="table table-striped table-bordered table-hover dataTable">
					...
					<thead>
						<tr role="row">
							<th aria-label="idestado: activate to sort column ascending" style="width: 80px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="0" role="columnheader" class="sorting" >ID</th>
...
jQuery(function($) {
	...
	function fillTable(){
		...
		$.post(obj.getValue(0) + "data/", {o:1, t:1, c:nc, p:55, from:0, cantidad:0,s:''},
			function(json){
					...
					$.each(json, function(i, item) {
						...
						tB +='';
						tB +='				<td class=" ">';
						tB +='					<a href="#" >'+padl(item.idestado,4)+'</a>';
						tB +='				</td>';
						tB +='				<td class=" " >'+item.estado+'</td>';
						tB +='				<td class=" ">';
						tB +='					<div class="hidden-phone visible-desktop action-buttons">';
						tB +='';
						tB +='						<a class="green modEstadoPro" href="#" id="idestado-'+item.idestado+'" >';
						tB +='							<i class="icon-pencil bigger-130"></i>';
						tB +='						</a>';
						tB +='	';
						tB +='						<a class="red delEstado" href="#"  id="delEstado-'+item.idestado+'" >';
					...
					$(".modEstadoPro").on("click",function(event){
					...
					$(".delEstado").on("click",function(event){
						event.preventDefault();
	...
	function getPropEstado(IdEstado){
		var nc = localStorage.nc; //.split("@");
		$.post(obj.getValue(0) + "cat-estados-prop/", {
				user: nc,
				idestado: IdEstado
			},
			function(html) {
