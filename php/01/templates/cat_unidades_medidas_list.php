	...
	<div id="user-profile-1" class="user-profile row-fluid">
			<div id="tbl_tcat_wrapper" class="dataTables_wrapper" role="grid">
				...
				<table aria-describedby="tbl_tcat_info" id="tbl_tcat" class="table table-striped table-bordered table-hover dataTable">
					...
					<thead>
						<tr role="row">
							<th aria-label="idunidadmedida: activate to sort column ascending" style="width: 80px;" aria-controls="tbl_tcat" tabindex="0" role="columnheader" class="sorting" >ID</th>
							...
							<th aria-label="unidad_medida: activate to sort column ascending" style="width: 200px;" aria-controls="tbl_tcat" tabindex="2" role="columnheader" class="sorting">DESCRIPCION </th>
...
jQuery(function($) {
	...
	function fillTable(){
		...
		$.post(obj.getValue(0) + "data/", {o:9, t:23, c:nc, p:55, from:0, cantidad:0,s:''},
			function(json){
					...
					$.each(json, function(i, item) {
						...
						tB +='';
						tB +='				<td>';
						tB +='					<a href="#" >'+padl(item.idunidadmedida,4)+'</a>';
						tB +='				</td>';
						tB +='				<td >'+item.clave+'</td>';
						tB +='				<td >'+item.unidad_medida+'</td>';
						tB +='				<td>';
						tB +='					<div class="hidden-phone visible-desktop action-buttons">';
						tB +='';
						tB +='						<a class="green modUniMedPro" href="#" id="idunidadmedida-'+item.idunidadmedida+'" >';
						tB +='							<i class="icon-pencil bigger-130"></i>';
						tB +='						</a>';
						tB +='	';
						tB +='						<a class="red delUniMed" href="#"  id="delUniMed-'+item.idunidadmedida+'" >';
					...
					$(".modUniMedPro").on("click",function(event){
					...
					$(".delUniMed").on("click",function(event){
						event.preventDefault();
						...
						if (resp){
							...
							//alert(arr[1]);
							obj.setIsTimeLine(false);
				            $.post(obj.getValue(0) + "data/", {o:9, t:2, c:arr[1], p:52, from:0, cantidad:0, s:''},
				            function(json) {
				            		if (json[0].msg=="OK"){
										onClickFillTable();
				        			}else{
	...
	function getPropUniMed(IdUniMed){
		var nc = localStorage.nc; //.split("@");
		$.post(obj.getValue(0) + "cat-unidad_medida-prop/", {
				user: nc,
				idunidadmedida: IdUniMed
			},
			function(html) {
