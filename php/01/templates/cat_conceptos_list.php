	...
	<div id="user-profile-1" class="user-profile row-fluid">
			<div id="sample-table-2_wrapper" class="dataTables_wrapper" role="grid">
				...
				<table aria-describedby="sample-table-2_info" id="sample-table-2" class="table table-striped table-bordered table-hover dataTable">
					...
					<thead>
						<tr role="row">
							<th aria-label="idconcepto: activate to sort column ascending" style="width: 80px;" colspan="1" rowspan="1" aria-controls="sample-table-2" tabindex="0" role="columnheader" class="sorting" >ID</th>
...
jQuery(function($) {
	...
	function fillTable(){
		...
		$.post(obj.getValue(0) + "data/", {o:25, t:10003, c:nc, p:10, from:0, cantidad:0,s:''},
			function(json){
					...
					$.each(json, function(i, item) {
						...
						tB +='';
						tB +='				<td class=" ">';
						tB +='					<a href="#" >'+padl(item.idconcepto,4)+'</a>';
						tB +='				</td>';
						tB +='				<td class=" " >'+item.concepto+'</td>';
						tB +='				<td class=" ">';
						tB +='					<div class="hidden-phone visible-desktop action-buttons">';
						tB +='';
						tB +='						<a class="green modConceptoPro" href="#" id="idconcepto-'+item.idconcepto+'" >';
						tB +='							<i class="icon-pencil bigger-130"></i>';
						tB +='						</a>';
						tB +='	';
						tB +='						<a class="red delConcepto" href="#"  id="delConcepto-'+item.idconcepto+'" >';
					...
					$(".modConceptoPro").on("click",function(event){
						event.preventDefault();
						var arr = event.currentTarget.id.split('-');
						obj.setIsTimeLine(false);
						getPropConcepto(arr[1]);
					});

					$(".delConcepto").on("click",function(event){
						event.preventDefault();
						...
						if (resp){
							//alert(arr[1]);
							obj.setIsTimeLine(false);
				            $.post(obj.getValue(0) + "data/", {o:25, t:2, c:arr[1], p:2, from:0, cantidad:0, s:''},
				            function(json) {
				            		if (json[0].msg=="OK"){
										onClickFillTable();
				        			}else{
	...
	function getPropConcepto(IdConcepto){
		var nc = localStorage.nc; //.split("@");
		$.post(obj.getValue(0) + "cat-conceptos-prop/", {
				user: nc,
