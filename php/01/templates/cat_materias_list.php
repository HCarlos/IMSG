...
jQuery(function($) {
	...
	function fillTable(){
		...
		$.post(obj.getValue(0) + "data/", {o:7, t:13, c:nc, p:10, from:0, cantidad:0,s:''},
			function(json){
					...
					$.each(json, function(i, item) {
						...
						tB +='';
						tB +='						<a class="green modMateriaPro" href="#" id="idmateria-'+item.idmateria+'" >';
						tB +='							<i class="icon-pencil bigger-130"></i>';
						tB +='						</a>';
						tB +='	';
						tB +='						<a class="red delMateria" href="#"  id="delMateria-'+item.idmateria+'" >';
					...
					$(".modMateriaPro").on("click",function(event){
						event.preventDefault();
						var arr = event.currentTarget.id.split('-');
						obj.setIsTimeLine(false);
						getPropMateria(arr[1]);
					});

					$(".delMateria").on("click",function(event){
						event.preventDefault();
						...
						if (resp){
							...
							//alert(arr[1]);
							obj.setIsTimeLine(false);
				            $.post(obj.getValue(0) + "data/", {o:7, t:2, c:arr[1], p:2, from:0, cantidad:0, s:''},
				            function(json) {
				            		if (json[0].msg=="OK"){
										onClickFillTable();
				        			}else{
	...
	function getPropMateria(IdMateria){
        $("#contentProfile").html("");
        $("#contentMain").hide(function(){
	        $("#preloaderPrincipal").show();
	        obj.setIsTimeLine(false);
	        var nc = localStorage.nc;
	        $.post(obj.getValue(0) + "cat-materias-prop/", {
				user: nc,
				idmateria: IdMateria
	            },
	            function(html) {
	                $("#contentProfile").html(html).show('slow',function(){
	                	//$("#contentProfile");
		                $('#breadcrumb').html(getBar('Inicio, Materias '));
	                });
	            }, "html");
        });
        return false;
