...
jQuery(function($) {
		...
		$.post(obj.getValue(0) + "data/", {o:6, t:76, c:nc, p:51, from:0, cantidad:0,s:''},
			function(json){
					...
					$.each(json, function(i, item) {
						...
						tB +='						</a>';
						tB +='	';
						tB +='						<a class="red delColor" href="#"  id="delColor-'+item.idcolor+'" >';
					...
					$(".modColorPro").on("click",function(event){
					...
					$(".delColor").on("click",function(event){
						event.preventDefault();
	...
	function getPropColor(IdColor){
        $("#contentProfile").html("");
        $("#contentMain").hide(function(){
	        $("#preloaderPrincipal").show();
	        obj.setIsTimeLine(false);
	        var nc = localStorage.nc;
	        $.post(obj.getValue(0) + "cat-colores-prop/", {
				user: nc,
				idcolor: IdColor
	            },
	            function(html) {
	                $("#contentProfile").html(html).show('slow',function(){
	                	//$("#contentProfile");
		                $('#breadcrumb').html(getBar('Inicio, Catálogo de Colores '));
	                });
	            }, "html");
        });
        return false;
