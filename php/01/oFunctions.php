...
class oFunctions {
	...
	public function time_stamp($session_time){
	    ...
	    $lRet = "Ahora"; //"En estos momentos";
	    if($seconds <= 0){
	        $lRet = "Ahora"; //"En estos momentos";
	    }else if($seconds > 1 && $seconds <= 59){
	       	$lRet = "Hace $seconds seg";
	    }else if($minutes <=60){
	                if($minutes==1){
	                    $lRet =  "Hace un min";
	                }else{
	                    $lRet = "Hace $minutes min";
	                }
	            }else if($hours <=24){
	                if($hours==1){
	                    $lRet = "Hace una hora";
	            ...
	            if($weeks==1){
	                $lRet = "Hace una semana";
	            }else{
	                $lRet = "Hace $weeks semanas";
	            }
	...
	function FormatCal($cal=0,$con=0,$ina=0,$pivot=0, $idmatclas=1){
		$calx = "";
		$conx = "";
		$inax = "";
		if ( $idmatclas == 1 || $idmatclas == 2 || $idmatclas == 3 ){

			if ($cal>0){
				//$calx = round(floatval($cal)/10,0);
				$calx = $cal;
			}
			switch (intval($calx)) {
				case 10:
			...
			if ( $pivot == 0 ){
				// return str_pad($calx, 3, " ", STR_PAD_LEFT).'  ( '.$conx.' )';
				return str_pad($calx, 3, " ", STR_PAD_LEFT).' '.str_pad($conx, 6, " ", STR_PAD_LEFT).' ';
			}else{
			...
			if ( floatval($cal) > 0){
				return str_pad("", 3, " ", STR_PAD_LEFT).'   '."ACREDITADA";
			}
	...
	function FormatCal2($cal=0,$con=0,$ina=0,$pivot=0, $idmatclas=1){
		$calx = "";
		$conx = "";
		$inax = "";
		if ( $idmatclas == 1 || $idmatclas == 2 || $idmatclas == 3 ){
			...
			if ( $pivot == 0 ){
				// return str_pad($calx, 3, "0", STR_PAD_LEFT).'  ( '.$conx.' )';
				return str_pad($calx, 3, " ", STR_PAD_LEFT).' '.str_pad($conx, 6, " ", STR_PAD_LEFT).' ';
			}else{
				return number_format($cal,1);
			}
	...
	function Letras($cal=0,$con=0,$ina=0){
		$calx = "";
		$conx = "";
		$inax = "";
		$calx = $cal;
